...
vec3 color(float grad) {
    ...
    float m2 = .15;//iMouse.z < 0.0001 ? 1.15 : iMouse.y * 3.0 / uResolution.y;
    grad =sqrt( grad);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    float m1 = 2.6;//iMouse.z < 0.0001 ? 3.6 : iMouse.x * 5.0 / uResolution.x;

    float t = uElapsedTime;
    vec2 uv = vTexCoord*uResolution / uResolution.yy;
    float ff = 1.0 - uv.y;
