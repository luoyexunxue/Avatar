...
CMeshData* CMeshLoaderCollada::LoadFile(const string& filename, uint8_t* data, size_t size) {
	...
	CMeshLoaderCollada::SContext context;
	...
	if (library_images) ReadImages(&context, library_images);
	if (library_materials) ReadMaterials(&context, library_materials);
	// 由于部分 Collada 文件骨骼节点未标明 type="JOINT"
	// 因此使用动画数据中的骨骼名称来确定哪些是骨骼节点
	if (library_animations) ReadAnimationJoint(&context, library_animations);
	if (library_visual_scenes) ReadVisualScenes(&context, library_visual_scenes, model_transform, pMeshData);
	if (library_controllers) ReadControllers(&context, library_controllers, pMeshData);
	if (library_geometries) ReadGeometries(&context, library_geometries, pMeshData);
	if (library_effects) ReadEffects(&context, library_effects, pMeshData);
	if (library_animations) ReadAnimations(&context, library_animations, pMeshData);
	delete[] xml;
	return pMeshData;
}
...
* 读取图片节点
*/
void CMeshLoaderCollada::ReadImages(SContext* context, xml_node<>* images) {
	...
	while (image) {
		...
		if (CFileManager::IsFullPath(init_from)) context->imageMap[id] = init_from;
		else context->imageMap[id] = context->baseDir + init_from;
...
* 读取材质节点
*/
void CMeshLoaderCollada::ReadMaterials(SContext* context, xml_node<>* materials) {
...
* 读取场景节点
*/
void CMeshLoaderCollada::ReadVisualScenes(SContext* context, xml_node<>* visualScenes, const CMatrix4& matrix, CMeshData* meshData) {
	// 读取场景数据
	xml_node<>* visual_scene = visualScenes->first_node();
	while (visual_scene) {
		ReadNode(context, visual_scene, matrix, 0);
		visual_scene = visual_scene->next_sibling();
	}
	// 添加关节信息
	map<string, SJoint*> jointMap;
	for (map<string, SJoint*>::iterator i = context->jointMap.begin(); i != context->jointMap.end(); ++i) {
...
* 读取控制节点
*/
void CMeshLoaderCollada::ReadControllers(SContext* context, xml_node<>* controllers, CMeshData* meshData) {
	...
	while (controller) {
		...
		if (skin) ReadSkin(context, skin, id, meshData);
...
* 读取几何节点
*/
void CMeshLoaderCollada::ReadGeometries(SContext* context, xml_node<>* geometries, CMeshData* meshData) {
	xml_node<>* geometry = geometries->first_node();
	while (geometry) {
		string id = geometry->first_attribute("id")->value();
		xml_node<>* mesh = geometry->first_node();
		if (mesh) ReadMesh(context, mesh, id, meshData);
...
* 读取效果节点
*/
void CMeshLoaderCollada::ReadEffects(SContext* context, xml_node<>* effects, CMeshData* meshData) {
	...
	while (effect) {
		...
		if (profile_COMMON) ReadProfile(context, profile_COMMON, id, meshData);
...
* 读取动画节点
*/
void CMeshLoaderCollada::ReadAnimations(SContext* context, xml_node<>* animations, CMeshData* meshData) {
	xml_node<>* animation = animations->first_node();
	while (animation) {
		xml_node<>* subset = animation->first_node("animation");
		if (subset) {
			while (subset) {
				ReadAnimationData(context, subset);
				subset = subset->next_sibling();
			}
		} else ReadAnimationData(context, animation);
...
* 解析节点数据
*/
void CMeshLoaderCollada::ReadNode(SContext* context, xml_node<>* node, const CMatrix4& matrix, SJoint* joint) {
	CMatrix4 transform;
	...
	while (current) {
		...
		if (!strcmp(name, "translate")) {
			...
			context->meshTransformMap.insert(std::pair<string, CMatrix4>(geometry, matrix * transform));
			xml_node<>* bind_material = current->first_node("bind_material");
			if (bind_material) MapMaterial(context, bind_material);
			...
			bool isJoint1 = attrib_id && context->jointNameMap.count(attrib_id->value()) != 0;
			...
			if (isJoint1 || isJoint2) {
				...
				pJoint->parent = joint;
				context->jointMap.insert(std::pair<string, SJoint*>(sid, pJoint));
				context->jointMapper.insert(std::pair<string, string>(id, sid));
				ReadNode(context, current, matrix * transform, pJoint);
			} else ReadNode(context, current, matrix * transform, joint);
		}
...
* 解析蒙皮数据
*/
void CMeshLoaderCollada::ReadSkin(SContext* context, xml_node<>* skin, const string& id, CMeshData* meshData) {
	const string mesh_id = skin->first_attribute("source")->value();
	xml_node<>* vertex_weights = skin->first_node("vertex_weights");
	context->vertexJointMap[mesh_id].resize(atoi(vertex_weights->first_attribute("count")->value()));
	// 获取绑定矩阵
	...
	if (bind_shape_matrix) {
		...
		context->meshTransformMap[mesh_id] = CMatrix4(mat).Transpose() * context->meshTransformMap[mesh_id];
	}
	// 获取输入源
	...
	vector<CVertexJoint>& vertexJoint = context->vertexJointMap[mesh_id];
	...
	for (size_t i = 0; i < vecJoint.size(); i++) {
		map<string, SJoint*>::iterator iter = context->jointMap.find(vecJoint[i]);
		if (iter != context->jointMap.end()) {
	...
	MapVertexJoint(context, mesh_id, vecJoint, meshData);
}

/**
* 解析网格数据
*/
void CMeshLoaderCollada::ReadMesh(SContext* context, xml_node<>* mesh, const string& id, CMeshData* meshData) {
	...
	if (!element) return;
	// 获取输入源
	vector<CVertexJoint>& vertexJoint = context->vertexJointMap["#" + id];
	...
	while (element) {
		...
		string material_name = element_material? context->materialMap[element_material->value()]: "";
		...
		pMesh->Transform(context->meshTransformMap["#" + id]);
...
* 解析材质数据
*/
void CMeshLoaderCollada::ReadProfile(SContext* context, xml_node<>* profile, const string& id, CMeshData* meshData) {
	string url = "#" + id;
		...
		if (material->GetName() == url) {
			...
			if (diffuse) {
				if (diffuse->first_node("texture")) {
					...
					if (context->imageMap.count(texture) > 0) {
						material->SetTextureBuffered(context->imageMap[texture]);
					}
				}
...
* 读取动画数据
*/
void CMeshLoaderCollada::ReadAnimationData(SContext* context, xml_node<>* animation) {
	// 获取输入源
	...
	while (channel) {
		...
		string joint = context->jointMapper[channel_target.substr(0, channel_target.rfind('/'))];
		string trans = channel_target.substr(channel_target.find('/') + 1);
		map<string, SJoint*>::iterator iter = context->jointMap.find(joint);
		if (iter != context->jointMap.end()) pJoint = iter->second;
		if (!pJoint) continue;
		// 不支持非矩阵的变换方式
...
* 读取动画关节
*/
void CMeshLoaderCollada::ReadAnimationJoint(SContext* context, xml_node<>* animations) {
	xml_node<>* animation = animations->first_node();
	while (animation) {
		xml_node<>* subset = animation->first_node("animation");
		if (subset) {
			while (subset) {
				...
				while (channel) {
					...
					channel = channel->next_sibling("channel");
					if (context->jointNameMap.count(joint) == 0) {
						context->jointNameMap.insert(std::pair<string, int>(joint, 0));
					}
				}
				subset = subset->next_sibling();
			}
...
* 映射材质名称
*/
void CMeshLoaderCollada::MapMaterial(SContext* context, xml_node<>* material) {
	...
	while (instance_material) {
		...
		context->materialMap[symbol] = context->materialMapper[target];
	}
}
...
* 映射顶点骨骼
*/
void CMeshLoaderCollada::MapVertexJoint(SContext* context, const string& id, const vector<string>& joint, CMeshData* meshData) {
